...
```
ML-Model-Monitoring/
├── backend/       FastAPI service (routes, services, utils, database)
├── models/        Trained model artifacts (.pkl) & the dynamic model registry
├── frontend/      Glassmorphic single-page dashboard (index.html)
├── datasets/      Loan approval CSVs + drift-dataset generator scripts
├── notebooks/     Exploratory model notebooks (Lasso, Random Forest, XGBoost)
...
To resolve predictive bias, models were retrained under logical credit risk assumptions enforced in [train_models.py](backend/train_models.py):
...
```bash
python train_models.py --data ../datasets/loan_approval_10000.csv
```
...
```
This generates `loan_approval_high_drift_demo.csv` in the `datasets/` folder for upload testing.
...
*   Open [frontend/index.html](frontend/index.html) directly in any browser to access the dashboard locally.
...
### Backend (Render)
The backend can be deployed as a **Web Service** on Render using:
*   **Environment**: Python
